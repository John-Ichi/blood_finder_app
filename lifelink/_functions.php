...
if (isset($_POST['submit_request'])) {
    ...
    $donor = $_POST['requested_donor'];
    $name = $_POST['rq_name'];
    $number = $_POST['rq_number'];
    $email = $_POST['rq_email'];
    $description = $_POST['rq_desc'];
    $urgent = $_POST['urgent'];
    $location = $_POST['point_of_donation'];
    $date = $_POST['date'];
    
    if ($_POST['rq_loc'] === '') {
        $sql =
            "INSERT INTO blood_requests
            (`donor_id`,`requester_name`,`contact_no`,`contact_email`,`request_description`,`urgent`,`point_of_donation`,`date_needed`)
            VALUES
            (?,?,?,?,?,?,?,?)";
        $submit_request = $conn->prepare($sql);
        $submit_request->bind_param('ssssssss',$donor,$name,$number,$email,$description,$urgent,$location,$date);
    } else {
        $sql =
            "INSERT INTO blood_requests
            (`donor_id`,`requester_name`,`contact_no`,`contact_email`,`request_description`,`urgent`,`hospital_id`,`point_of_donation`,`date_needed`)
            VALUES
            (?,?,?,?,?,?,?,?,?)";
        $submit_request = $conn->prepare($sql);
        $submit_request->bind_param('sssssss',$donor,$name,$number,$email,$description,$urgent,$_POST['rq_loc'],$location,$date);
    }

    $submit_request->execute();

    $mail->setFrom($lifelink_email);
    $mail->addAddress($_POST['requested_donor_email']);
    $mail->Subject = "LifeLink Blood Request";
    $mail->Body =
        "A request was made to you from " . $name . ".
...
if (isset($_GET['declineAppointment']) && $_GET['declineAppointment'] == true) { // Decline appointment
    ...
    header('Location: bloodbank-db.php');
}
...
function getDonationAppointments($sess_id) {
    ...
    $sql =
        ...
        ORDER BY status='Approved' DESC, date_of_donation ASC";
    $rs = $conn->query($sql);
